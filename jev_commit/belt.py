...
HIGH_PRECISION = [
    ...
    ("gitlab_token", re.compile(r"glpat-[\w-]{20}")),
    ("sendgrid_key", re.compile(r"SG\.[\w-]{22}\.")),
    ("npm_token", re.compile(r"npm_[A-Za-z0-9]{36}")),
    ("private_key", re.compile(r"-----BEGIN (RSA |EC |DSA |OPENSSH |PGP )?PRIVATE KEY-----")),
    ("jwt", re.compile(r"eyJ[\w-]{10,}\.eyJ")),
    ("url_credentials", re.compile(r"\w[\w+.-]*://[^/\s:@]+:[^/\s:@]+@")),
]
...
def added_lines(hunks):
    """[(path, text)] over added lines only, the header line excluded."""
    out = []
    for hunk in hunks:
