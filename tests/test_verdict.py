...
from jev_commit.questions import (DEBUG_LEFTOVERS, MESSAGE_IS_SUBSTANTIVE, MESSAGE_MATCHES_DIFF,
                                  SCOPE_CREEP, SECRET_SHAPED)
...
def answers(match=0.9, substantive=0.95, debug=0.02, scope=0.05, secret=0.02):
    return {MESSAGE_IS_SUBSTANTIVE: noul(substantive), MESSAGE_MATCHES_DIFF: noul(match),
            DEBUG_LEFTOVERS: noul(debug), SCOPE_CREEP: noul(scope), SECRET_SHAPED: noul(secret)}
...
def test_a_belt_hit_blocks_and_never_prints_the_secret():
    hits = belt.scan([{"path": "deploy/id_rsa", "text": "@@\n+-----BEGIN OPENSSH PRIVATE KEY-----"}])
...
def test_a_placeholder_message_suppresses_the_mismatch_finding():
    ...
    assert findings == []
    assert dict((label, status) for label, _, status in rows)["message matches diff"] == "ok"
...
def two_chunk_states(message):
    hunks = [{"path": "src/a.py", "text": "@@ -1,2 +1,3 @@\n+a = 1"},
             {"path": "src/b.py", "text": "@@ -1,2 +1,3 @@\n+b = 2"}]
    prep = {"files": [{"path": "src/a.py", "status": "M", "added": 1, "removed": 0},
                      {"path": "src/b.py", "status": "M", "added": 1, "removed": 0}],
            "hunks": hunks, "omitted": "", "more": ""}
...
def test_routing_asks_the_gate_once_and_takes_the_worse_answer(fake):
    ...
    assert MESSAGE_IS_SUBSTANTIVE not in log[1]["questions"], "the gate rides the first request only"
...
def test_a_too_big_response_splits_and_retries(fake):
    ...
    states = chunk.chunk_states("feat: things", {
        "files": [{"path": "src/%d.py" % i, "status": "M", "added": 1, "removed": 0} for i in range(8)],
        "hunks": [{"path": "src/%d.py" % i, "text": "@@ -1,2 +1,3 @@\n+value = %d" % i} for i in range(8)],
        "omitted": "", "more": ""})
    assert len(states) == 1
...
@pytest.mark.skipif(not (CORPUS / "split_01.json").exists(), reason="corpus not recorded yet")
...
@pytest.mark.skipif(not (CORPUS / "split_02.json").exists(), reason="corpus not recorded yet")
