...
Answers come from a flat fixture file, question name to answer object, in the shape the API
returns. Env knobs, all for tests: FAKE_JEV_FIXTURES picks the
...
def handler_for(fixtures, status=0, max_tokens=0, log=None):
    ...
    class Handler(BaseHTTPRequestHandler):
        def do_POST(self):
            ...
            answers = {
                name: table.get(name, DEFAULT) for name in body.get("questions", {})
            }
            self.reply(
                200,
                {
                    ...
                    "answers": answers,
                    "usage": {
                        "input_tokens": estimate(json.dumps(body.get("state", ""))),
                        "output_tokens": 0,
                    },
                },
            )
...
def serve(port=0, fixtures=None, status=0, max_tokens=0, log=None):
    return HTTPServer(
        ("127.0.0.1", port), handler_for(fixtures or {}, status, max_tokens, log)
    )
